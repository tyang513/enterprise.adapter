define([ "text!./templates/templateList.html", "app/base/BaseContentList", "app/sentiment/sentimentconfig/searchbar/templateSearchBar","text!app/sentiment/sentimentconfig/config/templateService.json" ], function(template, BaseContentList, searchBar, serviceConf) {
	app.serviceLocator.mergeConfig(serviceConf);
	return $.widget("app.templateList", BaseContentList, {
		// default options
		options : {
			entity : 'template',
			entityName : '模板',
			...
			app.messager.confirm(app.constants.CONFIRM_TITLE, app.constants.CONFIRM_DELETE + this.options.entityName + "?", _.bind(function(r) {
				if (r) {
					$.ajax(app.buildServiceData(this.options.entity + "/deleteById.do", {
						data : {
							id : data.row.id,
							templateCode : data.row.templateCode
						},
						context : this,
						success : function(data) {
							if (data) {
				...
				buttons : [ {
					...
					handler : _.bind(function(data,model) {
						...
						if (data.success && model=="create") {
							this._openTemplateForm(data.data);
						} else {
							this.refresh();
						}
					}, this)
		...
		_openTemplateForm : function(data) {
			console.log("templateForm");
			app.utils.openTab(this, {
				"title" : "模板--" + data.templateCode+"("+data.version+")",
				"url" : "app/sentiment/sentimentconfig/templateForm",
				"template" : data,
				"action" : "edit"
			});
