...
(function($) {
	...
	function getDisableFlag(opts, year, month, day) {
		var getDateValue = function(year, month, day) {
			return year * 10000 + month * 100 + day;
		};
		var flag = 1;
		var value = getDateValue(year, month, day);
		if (opts.minDate) {
			var minDate = _.isString(opts.minDate) ? new Date(opts.minDate) : opts.minDate;
			var minValue = getDateValue(minDate.getFullYear(), minDate.getMonth() + 1, minDate.getDate());
			flag = minValue <= value ? 1 : 0;
		}

		if (flag === 1 && opts.maxDate) {
			var maxDate = _.isString(opts.maxDate) ? new Date(opts.maxDate) : opts.maxDate;
			var maxValue = getDateValue(maxDate.getFullYear(), maxDate.getMonth() + 1, maxDate.getDate());
			flag = maxValue > value ? 1 : 0;
		}
		return flag;
	}
	...
	function getWeeks(target, year, month) {
		...
		for (var i = 1; i <= lastDay; i++) {
			dates.push([ year, month, i, getDisableFlag(opts, year, month, i) ]);
		}
		...
		if (firstWeek.length < 7) {
			while (firstWeek.length < 7) {
				...
				firstWeek.unshift([ date.getFullYear(), date.getMonth() + 1, date.getDate(),
						getDisableFlag(opts, date.getFullYear(), date.getMonth() + 1, date.getDate()) ]);
			}
		} else {
			var firstDate = firstWeek[0];
			var week = [];
	...
	function show(target) {
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				var day = week[j];
				$('<td class="calendar-day clendar-day-disabled calendar-other-month"></td>').attr('abbr', day[0] + ',' + day[1] + ',' + day[2] + "," + day[3])
		...
		t.find('td[abbr$=",1"]').removeClass('clendar-day-disabled');
		...
		t.find('td').hover(function() {
			...
			if (!$(this).hasClass('clendar-day-disabled')) {
				t.find('.calendar-selected').removeClass('calendar-selected');
				$(this).addClass('calendar-selected');
