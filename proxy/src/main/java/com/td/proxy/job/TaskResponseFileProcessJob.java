 ...
 * @author yangtao
 */
public class TaskResponseFileProcessJob implements Job, StatefulJob{

	/**
	 * 日志
	 */
	public Logger logger = LoggerFactory.getLogger(TaskResponseFileProcessJob.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		List<TaskResponseFileProcessJobInput> jobInputList = jobInputService.findReady2RunJobInput(ProxyConstant.JOB_TRIGGER_TYPE_SIMPLE);
		...
		for (TaskResponseFileProcessJobInput responseJobInput : jobInputList){
			try {
				...
				// 更新JobInput处理完成
				jobInputService.updateRspFileProcJobInputStatusById(responseJobInput.getId(), Constant.JOB_INPUT_PROCESSED);
				if (taskInfo != null){
